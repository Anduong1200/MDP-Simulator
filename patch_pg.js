...
if (!appJs.includes('let pg_policy = null;')) {
    appJs = appJs.replace('let currentTraces = null;', 'let currentTraces = null;\nlet pg_policy = null;');
}

// 2. Insert pg_algo.js before runAlgorithm
const runSetupStart = '// ── Run Setup ──';
if (!appJs.includes('runREINFORCE')) {
    appJs = appJs.replace(runSetupStart, pgJs + '\n\n' + runSetupStart);
}
...
let startIndex = appJs.indexOf(selectAlgoStart);
let endIndex = appJs.indexOf(selectAlgoEnd) + selectAlgoEnd.length;
if (startIndex !== -1 && endIndex !== -1) {
    appJs = appJs.substring(0, startIndex) + selectAlgoReplacement + appJs.substring(endIndex);
}

// 4. Update runAlgorithm
const runAlgoStart = `    const isLambda = ['td_lambda', 'sarsa_lambda'].includes(selectedAlgo);`;
const runAlgoEnd = `    if (result) {`;
...
startIndex = appJs.indexOf(runAlgoStart);
endIndex = appJs.indexOf(runAlgoEnd) + runAlgoEnd.length;
if (startIndex !== -1 && endIndex !== -1) {
    appJs = appJs.substring(0, startIndex) + runAlgoReplacement + appJs.substring(endIndex);
}

// 5. Update renderGrid to handle PG Policy arrows
...
if (!appJs.includes('drawStochasticArrow')) {
    appJs = appJs.replace(drawArrowCall, drawArrowReplacement);
    ...
    // Scale size and opacity based on probability
...
function drawArrow(r, c, action, cellDiv) {`;
    appJs = appJs.replace(drawArrowDef, drawStocArrowDef);
}


fs.writeFileSync(appJsPath, appJs);
console.log('PATCH PG SUCCESS');
